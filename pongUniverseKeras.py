...
while True:
    ...
    aprob_new = model_keras.predict(x.reshape((1, -1)))

    # which_action = 2 if np.random.uniform() < aprob else 3 # roll the dice!
    which_action = 2 if np.random.uniform() < aprob_new else 3 # roll the dice!
    action = []
    ...
    y = 1 if which_action == 2 else 0 # a "fake label"
